...
def create_data(filename1,filename2):
    ...
    for i in range(len(ones_x_train)+len(other_x_train)):
        if i < len(ones_x_train):
            ...
            y_mat_train[i] = -1.0
    x_mat_test, y_mat_test = shuffle(x_mat_test, y_mat_test, random_state=0)
    x_mat_train, y_mat_train = shuffle(x_mat_train, y_mat_train, random_state=0)
...
tmp = np.split(x_mat_train, [50])
x_mat_small_test = tmp[0]
x_mat_train = tmp[1]
#print x_mat_small_test.shape
#print x_mat_train.shape

tmp = np.split(y_mat_train,[50])
y_mat_small_test = tmp[0]
y_mat_train = tmp[1]
#print y_mat_small_test.shape
...
while t < 1000 and flag:
    ...
    for x in range(250):
        ...
        g[0] = g[0] + np.multiply(0.004,gradients[0])
        g[1] = g[1] + np.multiply(0.004,gradients[1])
    ...
    for x in range(250):
        ...
        ein += (0.001) * math.pow((outputs2[L]-current_y),2)
    ...
    test_ein = 0.0
    for x in range(50): 
        outputs = [np.expand_dims(x_mat_small_test[x], axis=1)]
        current_y = y_mat_small_test[x]
        ...
        for l in range (0,L):
            ...
            if l != L -1: 
                ...
                signal = theta(signal, user_input)  
            outputs.append(signal)
        test_ein += (0.005) * math.pow((outputs[L]-current_y),2)

    #print ("test_ein: {}".format(test_ein))
    #print ("optimal_ein: {}".format(optimal_ein))
    if test_ein < optimal_ein:
        optimal_ein = test_ein
    ...
    test_eins.append(math.log10(test_ein))
    #eins.append(old_ein)
    #test_eins.append(test_ein)
    ts.append(t)
...
plt.plot(ts, eins, color='blue', marker='o', linestyle='solid', linewidth=2.5, markersize=2.5)
plt.plot(ts, test_eins, color='red', marker='o', linestyle='solid', linewidth=2.5, markersize=2.5)
plt.plot(t2,test_eins[t2],color = 'green', marker = 'o',markersize = 1.4)
#plt.axvline(x=t2)

plt.xlabel('iterations')
plt.ylabel('E_in')
plt.savefig("12.2c1.pdf")
plt.show()
plt.clf()
